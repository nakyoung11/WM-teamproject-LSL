...
public class ShowListDAO {
	...
	// 전시회에 포함되는 작품 정보를 삽입하는 메소드
	public static int insShowList(ShowListVO param) {
		String sql = " INSERT INTO t_show_list " + " (i_show, i_work) " + " VALUES " + " (?, ?) ";
	...
	// 전시회에 포함된 작품 리스트를 가져오는 메소드.
	public static List<ShowListDomain> selShowList(ShowListVO param) {
		String sql = " SELECT B.i_work, B.i_user, B.work_title, B.work_images, B.work_ctnt" + " FROM t_show_list A"
	...
	public static List<ShowListDomain> selShowLikeCnt(ShowListVO param) {
		String sql = " SELECT COUNT(B.i_user) AS like_cnt " + " FROM t_show_list A" + " JOIN t_work_like B "
				+ " ON B.i_work = A.i_work " + " WHERE A.i_show = ? ";
	...
	public static int delShowList(ShowListDomain param) {
		String sql = " DELETE " + " FROM t_show_list" + " WHERE i_work = ?";
